...
contract PrecompilesTest is Test, UsingPrecompiles {
  function testTransfer() public {
    ...
    (success, ) = TRANSFER.call.value(0).gas(gasleft())(abi.encode(caller, dest, 500));
  ...
  function testFractionMul() public {
    ph.mockReturn(
      FRACTION_MUL,
      keccak256(abi.encodePacked(uint256(10), uint256(5), uint256(12), uint256(4), uint256(4), uint256(10))),
  ...
  function testProofOfPossession() public {
    ...
    ph.mockSuccess(PROOF_OF_POSSESSION, keccak256(abi.encodePacked(sender, blsKey, blsPop)));
    assert(checkProofOfPossession(sender, blsKey, blsPop));

    ph.mockRevert(PROOF_OF_POSSESSION, keccak256(abi.encodePacked(sender, blsKey, blsPop)));
    assert(checkProofOfPossession(sender, blsKey, blsPop));
  }
  ...
  function testGetValidator() public {
    ...
    ph.mockReturn(GET_VALIDATOR, keccak256(abi.encodePacked(index, block.number)), abi.encode(validator));
  ...
  function testGetValidatorAtBlock() public {
    ...
    address validator = address(0x989899);
    ph.mockReturn(GET_VALIDATOR, keccak256(abi.encodePacked(index, blockNumber)), abi.encode(validator));
    assert(validatorSignerAddressFromSet(index, blockNumber) == validator);
  }

  function testNumberValidatorsInSet() public {
    ph.mockReturn(NUMBER_VALIDATORS, keccak256(abi.encodePacked(block.number)), abi.encode(uint256(12)));
    assert(numberValidatorsInCurrentSet() == 12);
  }

  function testNumberValidatorsInSetAtBlock() public {
    uint256 blockNumber = 1000;
    ph.mockReturn(NUMBER_VALIDATORS, keccak256(abi.encodePacked(blockNumber)), abi.encode(uint256(12)));
  ...
  function testBlockNumberFromHeader() public {
    ...
    ph.mockReturn(BLOCK_NUMBER_FROM_HEADER, keccak256(abi.encodePacked(header)), abi.encode(blockNumber));
  ...
  function testHashHeader() public {
    ...
    ph.mockReturn(HASH_HEADER, keccak256(abi.encodePacked(header)), abi.encode(headerHash));
  ...
  function testGetParentSealBitmap() public {
    ...
    ph.mockReturn(GET_PARENT_SEAL_BITMAP, keccak256(abi.encodePacked(blockNumber)), abi.encode(bitmap));
  ...
  function testGetVerifiedSealBitmapFromHeader() public {
    bytes memory header = abi.encodePacked("MockHeader");
    bytes32 bitmap = bytes32(uint256(0x12345679));
    ph.mockReturn(GET_VERIFIED_SEAL_BITMAP, keccak256(abi.encodePacked(header)), abi.encode(bitmap));
